...
def main():
    ...
    playerOne = True  # Máy chủ là người chơi Trắng
    playerTwo = False  # Máy chủ không phải là người chơi Đen

    client_socket = connectToClient()  # Kết nối với client
    ...
    while True:
                    ...
                    if len(playerClicks) == 2:  
                        ...
                        for i in range(len(validMoves)):
                            if move == validMoves[i]:
                                ...
                                sqSelected = ()  
                                playerClicks = []
                                # Gửi nước đi đến client
                                data = pickle.dumps(validMoves[i])
                                client_socket.send(data)
                                print("Nước đi đã gửi:", validMoves[i].getChessNotation())
                        if not moveMade:
        ...
        # Đợi và nhận nước đi từ client
        data = client_socket.recv(BUFFER_SIZE)
        if data:
            received_move = pickle.loads(data)
            print("Nước đi nhận được từ client:", received_move.getChessNotation())
